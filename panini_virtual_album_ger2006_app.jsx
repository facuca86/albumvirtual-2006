...
import { db, doc, getDoc, setDoc, onSnapshot } from './firebase_GER2006';
import { playerNames } from './playerNames_GER2006';
import { teamThemes } from './teamThemes_GER2006';
import { albumConfig, albumPalette, codeToNumber, numberToCode } from './albumConfig_GER2006';
...
const PROYECTOS = [
  { id: 'paniniWorldCup2026',    label: 'Mundial 2026',             url: 'https://facuca86.github.io/albumvirtual/',        style: 'multicolor'     },
  { id: 'paniniWorldCup2022',    label: 'Mundial 2022 · Qatar',     url: 'https://facuca86.github.io/albumvirtual-2022/',   style: 'qatar'          },
  { id: 'paniniCWC2025',         label: 'Club World Cup 2025',      url: 'https://facuca86.github.io/albumvirtual-cwc25/',  style: 'cwc'            },
  { id: 'paniniRussia2018',      label: 'Mundial 2018 · Rusia',     url: 'https://facuca86.github.io/albumvirtual-2018/',   style: 'russia'         },
  { id: 'paniniBrazil2014',      label: 'Mundial 2014 · Brasil',    url: 'https://facuca86.github.io/albumvirtual-2014/',   style: 'brazil2014'     },
  { id: 'paniniSouthAfrica2010', label: 'Mundial 2010 · Sudáfrica', url: 'https://facuca86.github.io/albumvirtual-2010/',   style: 'southafrica2010' },
  { id: 'paniniGermany2006',     label: 'Mundial 2006 · Alemania',  url: 'https://facuca86.github.io/albumvirtual-2006/',   style: 'germany2006'    },
];

const getProyectoStyle = (style) => {
  switch (style) {
    case 'multicolor':    return { background: 'linear-gradient(135deg, #e53e3e, #dd6b20, #d69e2e, #38a169, #3182ce, #805ad5)', color: '#ffffff' };
    case 'qatar':         return { backgroundColor: '#6B0F1A', border: '2px solid #B8860B', color: '#ffffff' };
    case 'cwc':           return { backgroundColor: '#000000', border: '2px solid #B8860B', color: '#FFD700' };
    case 'russia':        return { backgroundColor: '#0E4CAC', border: '2px solid #D52B1E', color: '#ffffff' };
    case 'brazil2014':    return { backgroundColor: '#5FBFD8', border: '2px solid #9BC43A', color: '#2D7B2F' };
    case 'southafrica2010': return { backgroundColor: '#D6491F', border: '2px solid #B92714', color: '#F8E4B3' };
    case 'germany2006':   return { backgroundColor: '#0A839C', border: '2px solid #066F88', color: '#ffffff' };
...
const getInnerPanelClass = (teamCode, darkMode = false) => {
  ...
  return darkMode ? 'bg-[#065a70]' : 'bg-[#f7f5f2]';
};
...
const TAILWIND_HEX = {
  ...
  'amber-600':'#d97706','orange-400':'#fb923c','orange-500':'#f97316','rose-400':'#fb7185',
  'sky-200':'#bae6fd','sky-300':'#7dd3fc','sky-400':'#38bdf8','sky-500':'#0ea5e9',
...
export default function PaniniAlbumGer2006() {
  ...
  const stickerCount = albumConfig.specialSections[currentTeam]?.count
    ?? albumConfig.teamStickerCounts?.[currentTeam]
    ?? albumConfig.teamStickerCount;
  ...
  const stickers = useMemo(() => {
    ...
    const isTypeC   = albumConfig.typeCTeams?.includes(currentTeam);
    ...
    return Array.from({ length: stickerCount }, (_, i) => {
      ...
      if (section) {
        ...
        code       = `${currentTeam}${id}`;
        type       = isTypeC ? (id === 1 ? 'shield' : 'player') : (id === 2 ? 'shield' : 'player');
        label      = playerNames[currentTeam]?.[id - 1] || `Jugador ${id}`;
        horizontal = false;
      }
  ...
  const toggleSticker = (code) => {
    ...
    if (!current) {
      ...
      const newCount = Object.values(next).filter(isCompletedSticker).length;
  ...
  // ── Stats ──────────────────────────────────────────────────────────────────
  const completedCount    = Object.values(completed).filter(isCompletedSticker).length;
  ...
  const brilliantCodes          = [...albumConfig.brillanteCodes];
    ...
    for (const teamCode of teams) {
      ...
      if (section) {
        ...
        codes.forEach((code, i) => {
          const label = section.stickers
            ? section.stickers[i].label
            : section.playerNamesKey
              ? (playerNames[section.playerNamesKey]?.[i + 1] || `Jugador ${i + 1}`)
              : section.getLabel(i + 1);
        ...
        const count = albumConfig.teamStickerCounts?.[teamCode] ?? albumConfig.teamStickerCount;
        for (let id = 1; id <= count; id++) {
          entries.push({
            code:     `${teamCode}${id}`,
            label:    playerNames[teamCode]?.[id - 1] || `Jugador ${id}`,
            team:     teamCode,
  ...
  const searchResults = useMemo(() => {
    ...
    // número entero → búsqueda exacta por numberToCode
    ...
    return searchIndex.filter(e =>
      ...
      String(codeToNumber[e.code]).startsWith(ql) ||
  ...
  // ══════════════════════════════════════════════════════════════════════════
  return (
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-[#03303d] text-white' : 'bg-[#0A839C] text-slate-800'}`}>

      {/* ── HEADER ── */}
      <header className={`border-b shadow-sm sticky top-0 z-50 transition-colors duration-300 ${darkMode ? 'bg-[#05475a] border-[#066F88]' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto px-3 sm:px-6 py-2 sm:py-4 flex flex-row gap-2 justify-between items-center">
          <div className="min-w-0">
            ...
            <div className={`mt-0.5 sm:mt-2 text-xs sm:text-sm font-black ${darkMode ? 'text-cyan-400' : 'text-[#066F88]'}`}>
              {completionPercent}% COMPLETADO
            </div>
            <div className={`mt-1 sm:mt-2 h-2 sm:h-2.5 w-24 sm:w-56 rounded-full overflow-hidden ${darkMode ? 'bg-[#065a70]' : 'bg-slate-200'}`}>
              <div
                className="h-full rounded-full transition-all"
                style={{ width: `${completionPercent}%`, background: 'linear-gradient(to right, #0A839C, #066F88)' }}
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0 relative">
            {searchOpen && (
              <div className="relative flex items-center gap-1">
                <input
                  ...
                  className={`px-3 py-2 rounded-xl text-sm font-black border-2 w-32 sm:w-48 outline-none transition-all ${darkMode ? 'bg-[#065a70] border-[#0A839C] text-white placeholder-slate-500' : 'bg-white border-slate-300 text-slate-800 placeholder-slate-400'}`}
                />
                ...
                {searchResults.length > 0 && (
                  <div className={`absolute top-full right-0 mt-1 w-72 max-w-[calc(100vw-1.5rem)] rounded-2xl shadow-2xl overflow-hidden z-[200] ${darkMode ? 'bg-[#05475a] border border-[#0A839C]' : 'bg-white border border-slate-200'}`}>
                    {searchResults.map(entry => (
                      <button key={entry.code} onClick={() => handleSearchSelect(entry)}
                        className={`w-full px-4 py-2.5 text-left flex items-center gap-3 border-b last:border-b-0 transition-colors ${darkMode ? 'border-[#065a70] hover:bg-[#065a70] text-white' : 'border-slate-100 hover:bg-slate-50'}`}>
                        <span className="text-xl leading-none shrink-0">{entry.teamFlag}</span>
                        <div className="min-w-0">
                          <div className={`font-black text-xs uppercase ${darkMode ? 'text-cyan-400' : 'text-[#0A839C]'}`}>
                            #{codeToNumber[entry.code]}
                          </div>
            ...
            <button onClick={() => setSearchOpen(s => !s)} title="Buscar figurita"
              className={`px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black text-sm sm:text-base transition-colors duration-300 ${darkMode ? 'bg-white text-slate-900' : 'bg-[#066F88] text-white'}`}>
              🔍
            </button>
            ...
            <button onClick={() => setCurrentView('home')}
              className={`px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black text-sm sm:text-base transition-colors duration-300 ${darkMode ? 'bg-white text-[#066F88]' : 'bg-[#066F88] text-white'}`}>
              HOME
            </button>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
        ...
        {currentView === 'home' && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
            {[
              { label: 'Explorar Álbum', view: 'groups' },
              { label: 'Índice',         view: 'teams'  },
              { label: 'Estadísticas',   view: null, action: () => setShowStats(true) },
              { label: 'Repetidas',      view: 'repetidas' },
              { label: 'Otros Proyectos',view: 'otros-proyectos' },
            ].map(item => (
              <button key={item.label}
                onClick={() => item.action ? item.action() : setCurrentView(item.view)}
                className={`rounded-3xl p-8 shadow-xl text-left active:scale-95 transition-colors duration-300 ${darkMode ? 'bg-[#065a70] text-white' : 'bg-white'}`}>
                <div className="text-3xl font-black italic uppercase">{item.label}</div>
              </button>
            ))}
          </div>
        )}
      ...
      {currentView === 'repetidas' && (() => {
        ...
        return (
          <div className={`rounded-3xl p-6 sm:p-8 shadow-xl max-w-2xl mx-auto transition-colors duration-300 ${darkMode ? 'bg-[#065a70] text-white' : 'bg-white'}`}>
        ...
        {currentView === 'otros-proyectos' && (
          <div className={`rounded-3xl p-6 sm:p-8 shadow-xl max-w-2xl mx-auto transition-colors duration-300 ${darkMode ? 'bg-[#065a70] text-white' : 'bg-white'}`}>
            ...
            <button
              ...
              className={`mt-6 px-6 py-3 rounded-2xl font-black transition-colors duration-300 ${darkMode ? 'bg-[#03303d] text-white' : 'bg-gray-200 text-gray-800'}`}
            >← VOLVER</button>
          </div>
        )}
        ...
        {currentView === 'stats-selections' && (
          <div className={`rounded-3xl p-6 sm:p-8 shadow-xl max-w-4xl mx-auto transition-colors duration-300 ${darkMode ? 'bg-[#065a70] text-white' : 'bg-white'}`}>
            ...
            <button onClick={() => { setCurrentView('home'); setShowStats(true); }}
              className="mt-6 bg-[#066F88] text-white px-6 py-3 rounded-2xl font-black">VOLVER</button>
          </div>
        )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {teams.map(team => (
              <button key={team}
                ...
                className={`rounded-2xl p-4 shadow font-black italic active:scale-95 transition-colors duration-300 flex items-center gap-2 ${darkMode ? 'bg-[#065a70] text-white' : 'bg-white'}`}>
        ...
        {currentView === 'groups' && (
          <div
            ...
            style={{ background: 'radial-gradient(ellipse at center, #03303d, #0A839C, #05475a, #066F88)' }}
          >
            <div className="hidden lg:flex justify-between items-center mb-6">
              <button onClick={() => setCurrentView('home')} className="rounded-full px-6 py-3 shadow font-bold italic bg-white text-[#066F88]">HOME</button>
              <h2 className="text-3xl font-black italic uppercase text-white drop-shadow-lg">GRUPOS</h2>
              <button onClick={() => { setCurrentTeamIndex(0); setCurrentView('album'); }} className="rounded-full px-6 py-3 shadow font-bold italic bg-white text-[#066F88]">SIGUIENTE →</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              <button onClick={() => { setCurrentTeamIndex(0); setCurrentView('album'); }}
                ...
                style={{ backgroundColor: '#ffd700', color: '#03303d' }}>
                INTRO
              </button>

              <button onClick={() => { setCurrentTeamIndex(teams.indexOf('ESTADIOS')); setCurrentView('album'); }}
                className="col-span-2 rounded-2xl p-4 font-black text-2xl sm:text-3xl active:scale-95 transition-transform"
                style={{ backgroundColor: '#03303d', color: '#0A839C' }}>
                ESTADIOS
              </button>
        ...
        {currentView === 'groups' && (
          <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t shadow-lg transition-colors duration-300 ${darkMode ? 'bg-[#05475a] border-[#066F88]' : 'bg-white border-slate-200'}`}>
            <div className="flex">
              <button onClick={() => setCurrentView('home')}
                className={`flex-1 py-4 font-black italic text-sm border-r active:bg-slate-100 transition-colors ${darkMode ? 'border-[#066F88] text-white' : 'border-slate-200'}`}>HOME</button>
              <div className={`flex-1 border-r ${darkMode ? 'border-[#066F88]' : 'border-slate-200'}`} />
              <button onClick={() => { setCurrentTeamIndex(0); setCurrentView('album'); }}
          ...
          <div className={`lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t shadow-lg transition-colors duration-300 ${darkMode ? 'bg-[#05475a] border-[#066F88]' : 'bg-white border-slate-200'}`}>
            <div className="flex">
              <button onClick={() => currentTeam === teams[0] ? setCurrentView('groups') : prevTeam()}
                className={`flex-1 py-4 font-black italic text-sm border-r active:bg-slate-100 transition-colors ${darkMode ? 'border-[#066F88] text-white' : 'border-slate-200'}`}>← ANTERIOR</button>
              <button onClick={() => setCurrentView('teams')}
                className={`flex-1 py-4 font-black uppercase text-sm border-r active:bg-slate-100 transition-colors ${darkMode ? 'border-[#066F88] text-white' : 'border-slate-200'}`}>ÍNDICE</button>
      ...
      {showStats && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <div className={`rounded-3xl p-6 sm:p-8 shadow-2xl w-full max-w-md transition-colors duration-300 ${darkMode ? 'bg-[#065a70] text-white' : 'bg-white'}`}>
            ...
            <div className="space-y-3 font-black">
              ...
              <div>
                ...
                <div className={`w-full rounded-full h-3 ${darkMode ? 'bg-[#03303d]' : 'bg-slate-200'}`}>
                  <div className="h-3 rounded-full transition-all duration-500" style={{ width: `${completionPercent}%`, background: 'linear-gradient(to right, #0A839C, #066F88)' }} />
                </div>
              </div>
              <div>Me faltan: {remainingCount}</div>
              <div>Brillantes: {brilliantCompletedCount} / {albumConfig.brillanteCodes.size}</div>
            ...
            <div className={`mt-4 pt-4 border-t ${darkMode ? 'border-[#0A839C]' : 'border-slate-200'} flex flex-wrap gap-3`}>
              <button onClick={() => { setShowStats(false); setCurrentView('stats-selections'); }}
                className="bg-[#066F88] text-white px-6 py-3 rounded-2xl font-black">Estadísticas Selecciones</button>
              <button onClick={() => setShowStats(false)}
                className={`px-6 py-3 rounded-2xl font-black ${darkMode ? 'bg-[#03303d] text-white' : 'bg-slate-300 text-slate-800'}`}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className={`rounded-3xl px-4 pt-4 pb-24 sm:px-8 sm:pt-8 sm:pb-8 shadow-xl ${getTeamGradientClass(currentTeam)}`}>
      ...
      <div className="hidden lg:flex justify-between items-center mb-8 gap-4">
        <button onClick={onPrev}
          className={`rounded-full px-6 py-3 shadow font-bold italic transition-colors duration-300 ${darkMode ? 'bg-[#05475a] text-white border border-[#0A839C]' : 'bg-white text-black'}`}>
          ← ANTERIOR
        </button>
        <div className="text-center">
          <div className="flex items-center justify-center gap-3 flex-wrap">
            ...
            <button onClick={onIndex}
              className="bg-[#066F88] text-white px-4 py-2 rounded-2xl font-black uppercase text-lg sm:text-2xl leading-none">
              INDICE
            </button>
          </div>
          ...
          <div className={`mt-3 text-2xl font-black ${isDarkTeam ? 'text-white' : 'text-[#066F88]'}`}>
      ...
      {/* Inner panel */}
      <div className={`overflow-hidden rounded-[2rem] border-4 transition-colors duration-300 ${darkMode ? 'border-[#0A839C] bg-[#065a70]' : 'border-slate-200 bg-white'} grid lg:grid-cols-2`}>
        {currentTeam === 'INTRO' ? (
...
// ── Intro Panel ───────────────────────────────────────────────────────────────
// Todas las figuritas son brillantes e individuales (verticales)
function IntroPanel({ stickers, currentTeam, currentTeamInfo, darkMode, toggleSticker, justPastedCode, highlightCode }) {
  ...
  return (
    <div className={`col-span-2 p-4 sm:p-8 ${bgClass}`}>
      <div className="text-2xl sm:text-4xl font-black uppercase text-white mb-4 sm:mb-6">INTRO</div>
      <div className="grid grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-4 max-w-lg sm:max-w-full mx-auto">
        {stickers.map(s => (
...
// ── Estadios Panel ────────────────────────────────────────────────────────────
// Figuritas INDIVIDUALES (no pares), 3 columnas por panel
function EstadiosPanel({ stickers, currentTeam, darkMode, toggleSticker, justPastedCode, highlightCode }) {
  ...
  return (
    <>
      ...
      {/* Desktop left */}
      <div className={`hidden lg:block p-8 border-r transition-colors duration-300 ${darkMode ? 'border-[#0A839C]' : 'border-slate-300'} ${bgClass}`}>
      ...
      <div className={`hidden lg:block p-8 ${bgClass}`}>
        <div className="grid grid-cols-3 gap-4 mt-[4.5rem]">
...
function TeamPanel({ stickers, currentTeam, currentTeamInfo, darkMode, toggleSticker, justPastedCode,
  ...
  const GroupBox = () => {
    ...
    return (
      <div className="border-2 rounded-2xl p-2 flex flex-col justify-center"
        style={darkMode ? { backgroundColor: '#03303d', borderColor: '#0A839C' } : { backgroundColor: 'rgba(255,255,255,0.6)', borderColor: '#cbd5e1' }}>
  ...
  return (
    <>
      ...
      {/* Desktop left */}
      <div className={`hidden lg:block p-3 sm:p-8 border-b lg:border-b-0 lg:border-r transition-colors duration-300 ${darkMode ? 'border-[#0A839C]' : 'border-slate-300'} ${bgClass}`}>
...
function Sticker({ sticker, onToggle, currentTeam, darkMode = false, justPasted = false, highlighted = false }) {
  const isPlayerSticker = sticker.type === 'player' || sticker.type === 'estadio' || sticker.type === 'intro';
  const isShieldSticker = sticker.type === 'shield';
  const isBrillante     = albumConfig.brillanteCodes?.has(sticker.code);
  ...
  const emptyBg     = darkMode ? 'bg-[#03303d] border-[#0A839C]' : 'bg-white border-slate-300';
  ...
  // Brillante: gradiente dorado cuando no está completada, borde amarillo siempre
  const brillanteUncompleted = isBrillante && !sticker.completed && !sticker.repeated;
  const brillanteStyle = brillanteUncompleted ? {
    background: 'linear-gradient(135deg, #ffd700, #fffacd, #daa520, #ffd700, #b8860b)',
    borderColor: '#facc15',
  } : (isBrillante && !sticker.repeated ? { borderColor: '#facc15' } : undefined);

  const bgClass = sticker.repeated
    ? repeatedBg
    : brillanteUncompleted
    ? ''
    : sticker.completed
    ? completedBg
    : emptyBg;

  const borderSizeClass = sticker.completed || sticker.repeated ? 'border-[4px] scale-[1.02]' : 'border-2';
  const borderColorClass = (isBrillante && !sticker.repeated) ? 'border-yellow-400' : '';

  const animClass = justPasted ? 'sticker-paste' : highlighted ? 'sticker-pulse' : '';

  // Número visible (codeToNumber) o código si no existe en el mapa
  const visibleNumber = codeToNumber[sticker.code] ?? sticker.code;

  return (
    <button
      onClick={() => onToggle(sticker.code)}
      style={brillanteStyle}
      className={`relative border-2 rounded-xl sm:rounded-2xl p-2 sm:p-4 w-full flex items-center justify-center text-center transition active:opacity-60 aspect-[2/3] ${bgClass} ${borderSizeClass} ${borderColorClass} ${animClass}`}
    >
      {isPlayerSticker && !isShieldSticker && (
      ...
      {isBrillante && !sticker.completed && !sticker.repeated && (
        <div style={{ position: 'absolute', top: 2, right: 4, fontSize: '10px', zIndex: 2 }}>⭐</div>
      )}
      <div style={{ position: 'relative', zIndex: 1 }}>
        <div className={`text-[9px] sm:text-xs uppercase break-all ${sticker.repeated ? repeatedCodeClass : sticker.completed ? 'text-black font-extrabold' : isBrillante ? 'text-yellow-800 font-extrabold' : 'text-slate-400 font-black'}`}>
          {visibleNumber}
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black/70 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-6 shadow-2xl flex flex-col items-center gap-4 max-w-xs w-full">
        ...
        <button onClick={onClose} className="bg-[#066F88] text-white px-6 py-3 rounded-2xl font-black w-full">Cerrar</button>
      </div>
    </div>
  );
}
...
function RepeatidasView() {
  ...
  if (!stickerData) {
    return (
      <div className="min-h-screen bg-[#0A839C] flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-[#0A839C]">
          ...
          <div key={team} className="bg-white rounded-2xl p-4 shadow">
            ...
            <div className="flex flex-wrap gap-1.5">
              {codes.map(code => {
                ...
                return (
                  <span key={code} className="bg-[#0A839C] text-white text-xs font-black px-2.5 py-1 rounded-lg">
                    #{num}{name !== code ? ` · ${name}` : ''}
                  </span>
                );
              })}
...
function CelebrationModal({ celebration, teamData, teamThemes, getThemeKey, getTeamConfettiColors, onClose }) {
  ...
  const gradientClass = isAlbum
    ? 'from-cyan-800 via-yellow-500 to-cyan-700'
    : theme?.gradient || 'from-cyan-700 to-cyan-800';

  const confettiColors = isAlbum
    ? ['#0A839C', '#066F88', '#ffd700', '#ffffff', '#b8860b']
